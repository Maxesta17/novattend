...
- **Sintoma:** Nadine reporta que la app "se queda sin conexion" a menudo. Myriam, Samuel y Stephanie igual.
- **Causa raiz encontrada por el swarm (no la que se sospechaba al principio):** `LoginPage.jsx` tenia un `Promise.race` de **8000ms**, el timeout mas corto de la app y **por debajo** del `networkTimeoutSeconds:10` del Service Worker. El login se rendia justo antes de que el fallback a cache del SW pudiera actuar, asi que la red de seguridad offline era inalcanzable precisamente en el login. Con p75 medido de 10,7s, mas de un tercio de los logins fallaba en duro cuando la peticion iba a funcionar. Por eso el LOG registraba rafagas de LOGIN (Myriam 5 en 63 segundos): el backend SI procesaba el login, pero tarde.
- **Orquestacion:** swarm de 10 agentes en 5 fases con ruteo de modelos — opus en diagnostico de latencia y sintesis/arquitectura, sonnet en implementacion (2 territorios disjuntos: `src/` y `.github/`+`apps-script/`), 3 revisores adversariales (correccion, seguridad, UX/copy) y verificador. 0 errores, ~40 min.
- **Cambios frontend:** eliminado el race de 8s (no sustituido por otro numero: ya hay dos capas calibradas debajo); `apiGet` reintenta UNA vez con 12s por intento ante timeout, red caida, 5xx o 404/403 con cuerpo HTML del borde de Google. **`apiPost` NO se reintenta** (no es idempotente; reintentar un login pagaria PBKDF2 dos veces por trabajo ya hecho y podria rozar el lockout anti-bruteforce). Copy honesto en banner y login: se distingue "el servidor va lento" de "sin conexion" via `navigator.onLine`. Aviso progresivo a los 6s con `aria-live`.
- **Observabilidad:** `.github/workflows/canario-externo.yml` nuevo — mide desde runners de GitHub (FUERA de la red de Google, que es el camino que sufre el profesor), 5 muestras cada 15 min, alerta por mediana >10000ms o >=2 muestras no sanas, una sola issue persistente con etiqueta `canario-degradado`. La URL va en el secret `CANARIO_PING_URL` (ya configurado), nunca en el YAML. Canario interno: umbral de 15000ms (muerto — sus medidas reales van de 1188 a 2111ms, no podia dispararse nunca) a **5000ms**, y la URL `/exec` sale del codigo versionado a Script Property obligatoria sin fallback (fallo ruidoso a proposito).
- **Dos fallos que los revisores del swarm NO detectaron, encontrados al verificar:** (1) la etiqueta `canario-degradado` no existia en el repo, asi que `gh issue create --label` habria fallado justo cuando la alerta hacia falta — arreglado con `gh label create --force`; (2) el job no hace `checkout`, asi que `gh` no podia deducir el repo y moria con "not a git repository" — arreglado fijando `GH_REPO`. El segundo solo se vio ejecutando el workflow de verdad, no leyendo el YAML.
- **Confirmacion independiente del diagnostico:** primer run real del canario externo desde GitHub: `000/30001ms(timeout)`, `18454ms`, `1203ms`, `1359ms`, `13377ms`. Mediana 13,4s desde una red que no es la del desarrollador — descarta que la lentitud medida fuera de su conexion. Issue #7 abierta automaticamente.
- **Verificado:** lint 0, **294 tests / 47 suites** verdes, build OK (precache 16 entradas). Cero estilos inline. Bundle de produccion comprobado por descarga: contiene los textos nuevos.
- **Desplegado:** frontend en Vercel (auto-deploy por push a main, verificado sirviendo el bundle nuevo) y backend con `clasp push` (9 archivos). SIN redeploy: `doGet`/`doPost` no cambiaron y los triggers ejecutan HEAD.
- **Coste asumido conscientemente:** el peor caso de espera sube de 20s a 24,5s (12 + 0,5 + 12). A cambio, el caso que antes moria en seco tiene una segunda oportunidad.
- **Deuda:** `LoginPage.jsx` (249 lineas) y `src/services/api/http.js` (250) quedan justo en el limite de 250 de CLAUDE.md — el proximo anadido obliga a fragmentar. La latencia del borde de Google NO se ha eliminado (no se puede desde el codigo): sigue viva la decision de producto de migrar el backend fuera de Apps Script.
- **Siguiente paso sugerido:** fijar `CANARIO_URL` (obligatorio), y con una semana de datos del canario externo decidir sobre la migracion del backend.
...
- **Verificado contra la hoja de produccion:** `prof-maria` = Maria Wolf, email en PROFESORES col C, activo TRUE, rol teacher.
